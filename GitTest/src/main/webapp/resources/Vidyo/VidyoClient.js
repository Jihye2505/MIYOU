...
function VidyoClientInitialize($) {
	...
	if(isIE) {
		...
		try{
			VCUtils.pluginAvailable = new ActiveXObject("Vidyo.VidyoClientPlugIn");
		}catch(err){
		...
		VCUtils.webRTCAvailable = navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia;
		/* Global variable that stores the address of the WebRTC server */
		VCUtils.webRTCServer = "https://webrtc.prod.vidyo.io";

		/* check for plugin support */
		navigator.plugins.refresh(false);
		VCUtils.pluginAvailable = navigator.mimeTypes["application/x-vidyoclientplugin"];
	}
	...
	var OnLoadSuccess_ =  function(){
		...
		VC = new VidyoClient("VidyoClientPlugIn", OnLoadedCallback_);	
	}
	...
	var GetDownloadLinks = function(status) {
		...
		if (isMac) {
			...
			downloadPathPlugIn = downloadPath + "/VidyoClientPluginInstaller.dmg";
		} else if (isWin) {
	...
	if (VCUtils.params.plugin != "false" && VCUtils.windowParams.plugin != "false" && VCUtils.pluginAvailable) {
